...
function LessonWorkspace({ lessonId, onBack, onToast }: { lessonId: number; onBack: () => void; onToast: (message: string) => void }) {
  ...
  const phaseIndex = { immersion: 0, transition: 1, genesis: 2 }[phase];
  return <><div className="lesson-header"><div><button className="back-button" onClick={onBack}><ArrowLeft size={14} /> Back to Unit I</button><span className="eyebrow" style={{ marginTop: 22 }}>Lesson 0{lesson.id} · Group C</span><h1>{lesson.title}</h1><p>{lesson.description}</p></div></div>
    <div className="phase-nav">{(['immersion', 'transition', 'genesis'] as Phase[]).map((key, index) => <button className={`phase-tab ${phase === key ? 'active' : ''}`} key={key} onClick={() => setPhase(key)} data-testid={`button-phase-${key}`}><span className="phase-code">0{index + 1} · {key}</span><strong>{key === 'immersion' ? 'Build the plan' : key === 'transition' ? 'Learn with support' : 'Evaluate & reflect'}</strong></button>)}</div>
    <div className="phase-content">
      {phase === 'immersion' && <div className="planning-grid"><section className="planning-sheet surface"><span className="section-index">Immersion phase · planning sheet</span><h2>Before the video,<br />place your attention.</h2><p>Choose a deliberate route through the lesson. A plan does not predict the learning perfectly; it gives you something to notice when the route changes.</p><div className="prompt-row"><div className="prompt-card"><span className="micro">01 · intention</span><p>What do you already associate with educational psychology?</p></div><div className="prompt-card"><span className="micro">02 · signal</span><p>What will tell you that a new concept has become clear?</p></div></div><div className="begin-row"><p>Planning is recorded locally for this meeting demo.</p><button className="solid-button" onClick={() => { setStarted(true); setPhase('transition'); onToast('Immersion complete — entering Transition.'); }} data-testid="button-begin-transition">Begin Transition <ArrowRight size={15} /></button></div></section><StrategyPanel strategy={strategy} onStrategy={setStrategy} /></div>}
      {phase === 'transition' && <TransitionView started={started} lesson={lesson} onToast={onToast} onEvaluate={() => setPhase('genesis')} />}
      {phase === 'genesis' && <EvaluationView evaluation={evaluation} setEvaluation={setEvaluation} onComplete={() => onToast('Evaluation recorded locally. This lesson is complete.')} />}
    </div>
